...
impl World {
    pub fn is_solid_feature_at_coord(&self, coord: Coord) -> bool {
        let cell = self.spatial.layers_at_checked(coord);
    ...
    pub fn is_wall_at_coord(&self, coord: Coord) -> bool {
        if let Some(spatial_cell) = self.spatial.layers_at(coord) {
    ...
    pub fn get_character_at_coord(&self, coord: Coord) -> Option<Entity> {
        self.spatial.layers_at(coord).and_then(|cell| cell.character)
    }
